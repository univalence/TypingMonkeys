...
(defn send! [id data]
  (println "send! " (pr-str (str "webView.send('" (pr-str data) "')")))
  (fx/on-fx-thread
...
(comment
 ...
 (pr-str "zer")
 (fx/on-fx-thread
  (.executeScript (val (first (deref engines)))
                  (str "webView.send('" (pr-str [:put :value "pouet"]) "')")))

 (str "webView.send('" (pr-str [:put :value "pouet"]) "')"))


#_(read-string (str "\"" (clojure.string/replace (pr-str [:yo "zer"])
                                      "\""
                                      "\\\""
                                      )
                  "\""))