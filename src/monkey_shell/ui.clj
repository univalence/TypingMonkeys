                                    ...
                                    :text        (str "<NAME>:<DIR>$ "
                                                      (str/join " " (-> (data/focused-session state)
                                                                        ...
                                                                        :cmd-args)) "\n\n" (-> (data/focused-session state)
                                                                                               :history last :out str))} {:fx/type  :h-box
                                                                                                                          :children [{:fx/type     :label
                                                                                                                                      :style-class "app-code"
                                                                                                                                      :text        "<NAME>:<DIR>$"}
                                                                                                                                     {:fx/type :h-box :children [{:fx/type         :text-field
                                                                                                                                                                  ...
                                                                                                                                                                  :on-text-changed {:event/type :ui.session.set-input}}]}]}]} (comps/vbox [(comps/squared-btn {:pref-width 30 :text "⚙"} :ui.popup.shell-settings)
                                                                                                                                                                                                                                           (pending-cmds state)])])})
(defn session [state]
  {:fx/type :stage
   :showing true
